...
var forTest = ['pizza', 'pineapple', 'orange', 'carrot', 'banana']
for(var i = 0; i < forTest.length; i++){ 
alert(forTest[i].toUpperCase().substring(0,1) + forTest[i].toLowerCase().substring(1, forTest[i].length));
};
...
var sentence = '';
for (var i = 0; i < forTest.length; i++){
    sentence = sentence + (forTest[i])+ ' '.split(',');
};
alert(sentence);
